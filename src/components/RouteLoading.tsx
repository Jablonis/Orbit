...
export function RouteLoading({ label = "Loading Orbit" }: { label?: string }) {
  return (
    ...
    >
      ...
      <div className="fixed left-0 top-0 hidden h-screen w-[112px] border-r border-[rgba(244,235,221,0.1)] bg-card/86 md:block" />
      <section className="page-container py-7">
        <div className="skeleton-shimmer h-3 w-28 rounded-full bg-primary/20" />
        <div className="skeleton-shimmer mt-4 h-11 w-full max-w-lg rounded-xl bg-[rgba(244,235,221,0.1)]" />
        ...
        <div className="mt-8 grid gap-4 sm:grid-cols-2 xl:grid-cols-12">
          {skeletonCardClasses.map((spanClass, index) => (
            <div
              className={`rounded-2xl bg-card shadow-[0_1px_2px_rgba(27,26,31,0.05)] skeleton-shimmer h-48 rounded-2xl bg-[rgba(244,235,221,0.025)] sm:col-span-2 ${spanClass}`}
              key={`${spanClass}-${index}`}
            />
          ))}
        </div>
